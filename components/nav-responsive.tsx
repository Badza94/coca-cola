...
import MoveUpRight from "./icons/move-up-right";
import { COLORS } from "@/types/enum";
import { cn } from "@/lib/utils";

export const Nav = ({ bgColor }: { bgColor: string }) => {
  ...
  return (
    <>
      ...
      />
      <AnimatePresence>
        {active && <LinksOverlay setActive={setActive} bgColor={bgColor} />}
      </AnimatePresence>
    </>
  );
};
...
const LinksOverlay = ({
  ...
  return (
    <nav
      className="fixed right-5 top-5 z-40 h-[calc(100vh_-_32px)] w-[calc(100%_-_32px)]
        overflow-hidden"
    >
      {/* <Logo /> */}
      <LinksContainer setActive={setActive} bgColor={bgColor} />
      <FooterCTAs setActive={setActive} bgColor={bgColor} />
    </nav>
  );
};
...
const NavLink = ({
  ...
  return (
    <motion.a
      ...
      href={href}
      className={cn(
        "block text-5xl font-semibold transition-colors text-white md:text-7xl",
        bgColor === COLORS.RED
          ? "redNavLink"
...
const Logo = () => {
  return (
    <motion.a
      ...
      exit={{ opacity: 0, y: -12 }}
      href="#"
      className="grid h-10 w-10 place-content-center rounded-br-2xl rounded-tl-2xl
        hover:bg-white/20 transition-colors"
    >
      <svg
        width="40px"
        height="40px"
        viewBox="0 -269.02099999999996 800 800"
  ...
  return (
    <>
      <motion.div
        ...
        className={cn(
          "fixed z-10 rounded-2xl bg-gradient-to-br shadow-lg ",
          bgColor === COLORS.RED
            ? "blackBg"
            : bgColor === COLORS.BLACK
              ? "redBg"
      ...
      <motion.button
        ...
        className={`group fixed right-[10px] top-[5px] z-50 h-12 w-12 bg-white/0 transition-all
        hover:bg-white/20 ${active ? "rounded-bl-2xl rounded-tr-2xl" : "rounded-2xl"}`}
      >
        <motion.span
          ...
          style={{
            ...
            left: "50%",
            x: "-50%",
            backgroundColor: bgColor === COLORS.LIGHT ? "black" : "white",
          }}
        />
        <motion.span
        ...
        <motion.span
          ...
          className="absolute block h-1 w-[10px] bg-white"
          style={{
            x: "-50%",
            y: "50%",
            backgroundColor: bgColor === COLORS.LIGHT ? "black" : "white",
  ...
  return (
    <>
      <motion.button
        ...
        exit={{ opacity: 0, y: 8 }}
        className={cn(
          "absolute bottom-2 right-2 flex items-center gap-2 rounded-full px-3 py-3 text-4xl uppercase  transition-colors duration-300  hover:border shadow-lg  md:bottom-4 md:right-4 md:px-6 md:text-2xl",
          bgColor === COLORS.RED
            ? "redNav"
            : bgColor === COLORS.BLACK
              ? "blackNav"
